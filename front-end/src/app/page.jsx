...
import GraficoColunasSatisfacao from "@/components/GraficoColunasSatisfacao";
...
export default function Home() {
  ...
  const [responseDataSatisfactions, setResponseDataSatisfactions] = useState({});
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:5000/api/satisfacoes")
      .then((response) => {
        const data = response.data;
        setResponseDataSatisfactions(data);
      })
      .catch((error) => console.error("Erro na requisição:", error));
  }, []);
    ...
    <div className="h-full flex flex-row gap-5 p-5 w-full items-center bg-gray-200">
      ...
      <div className="flex flex-col gap-5 h-full w-1/2 justify-center items-center ">
        <div className=" bg-white rounded-lg h-1/2 w-full pt-3 shadow-md flex flex-grow flex-col items-center">
          ...
          {selectedSector && (chartType === "pain" || chartType === "posture" || chartType === "health") && (
          ...
          {selectedSector && (chartType === "satisfaction" || chartType === "workplace") && (
              <div className="w-full flex flex-row text-left justify-between items-start px-4 pb-1 mb-2 font-bold">
                Nível médio de satisfação com a vida por departamentos
                {seletorGrafico}
              </div>
          )}
          ...
          {selectedSector && (chartType === "satisfaction" || chartType === "workplace") && (
              <div className="rounded-md justify-center w-full h-full">
                <GraficoColunasSatisfacao data={responseDataSatisfactions} />
              </div>
          )}
